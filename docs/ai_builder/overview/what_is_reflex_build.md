...
Reflex Build 是一个由 AI 驱动的平台，让任何人都可以通过用通俗易懂的语言描述想法来创建全栈 Web 应用——无需编写代码。它包含一个功能完备的内置 IDE、实时协作和项目共享——全部在浏览器中完成，无需安装。
...
Reflex Build 提供了一个简洁的界面来构建 AI 应用。**项目菜单栏**帮助你管理会话和存储的变量，而**聊天区域**实时显示提示词、编辑和文件生成。**应用工作区**组织你的项目结构，**代码编辑器**允许直接、即时地编辑代码。部署和共享等关键操作可通过**底部菜单栏**访问，**预览选项卡**让你随时查看和与实时应用交互。
...
Reflex Build 的界面专为清晰和高效而设计。**项目菜单栏**帮助你管理会话、应用和变量。**聊天区域**通过视觉反馈和文件生成展示提示词的执行过程。在**应用工作区**中，你可以查看和组织项目文件。**代码编辑器**允许快速、直接地编辑并即时保存。使用**底部菜单栏**执行部署和下载等关键操作。**预览选项卡**让你与应用的实时版本交互，包括刷新和全屏选项。
