...
任何 [CSS](https://developer.mozilla.org/en-US/docs/Web/CSS) 属性都可以在 Reflex 组件中使用。以下是常用属性的简短列表。要查看所有可用的 CSS 属性，请查阅此[文档](https://developer.mozilla.org/en-US/docs/Web/CSS)。

CSS 属性名中的连字符可以用下划线替代，以作为有效的 Python 标识符使用，例如 CSS 属性 `z-index` 在 Reflex 中应使用为 `z_index`。
